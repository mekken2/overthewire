# Get a private SSH key that can be used to log into the next level

The password for the next level is stored in `/etc/bandit_pass/bandit14` and can only be read by user bandit14. For this level, you don’t get the next password, but you get a private SSH key that can be used to log into the next level. <br>

The key is stored in `sshkey.private`, which looks like
`-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----` <br>

we can use a program PuTTYgen here which gets installed at the same time we install PuTTY to save the private key in a file and then use it to login for bandit14. [source](https://devops.ionos.com/tutorials/use-ssh-keys-with-putty-on-windows/)